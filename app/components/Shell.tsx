...
export function Shell({ title, subtitle, status, children, brand }: ShellProps) {
      ...
      <div className="relative w-full max-w-3xl rounded-3xl bg-slate-900/85 backdrop-blur-xl border border-slate-700/80 shadow-[0_22px_70px_rgba(0,0,0,0.75)] px-6 py-6 md:px-10 md:py-8">
        <header className="mb-6 flex items-center gap-4">
          <div className="h-11 w-11 rounded-2xl bg-slate-950 flex items-center justify-center overflow-hidden border border-slate-700">
            {logoEnabled && logoSrc ? (
              <Image src={logoSrc} alt={brandName} width={logoW} height={logoH} priority />
            ) : (
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400 truncate">{brandName}</p>
            {subtitle ? <p className="text-xs text-slate-400 mt-0.5">{subtitle}</p> : null}
          </div>
        </header>

        <div className="mb-4">
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-slate-50">{title}</h1>
          {status ? (
        ...
        <div className="space-y-4 text-slate-100 text-sm md:text-[15px]">{children}</div>
